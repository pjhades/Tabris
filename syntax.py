...
# reserved words
RESERVED = ['define', 'if', 'cond', 'else', 'let', 'quote', \
            'begin', 'lambda', 'set!', 'let*', 'letrec']
...
def check_define(expr):
    """
        Check syntax violation for define
    """
    if len(expr) < 3:
...
def check_lambda(expr):
    """
        Check syntax violation for lambda
    """
    if len(expr) < 3:
...
def check_cond(expr):
    """
        Check syntax violation for cond
    """
    if len(expr) < 2:
...
def check_let(expr):
    """
        Check syntax violation for let
    """
    if len(expr) < 3:
...
def check_dotted_pair(expr):
    """
        Check syntax violation for dotted pair notations
    """
